...
ПРОВЕРКА: после деплоя в логах воркера на реальной записи должны появиться строки
`✓ сегмент принят: ...` с logprob ниже -1.15 (раньше эти сегменты стирались как
`⊘ галлюцинация отброшена`). Если транскрипт стал нормальным — IGNORE уйдёт.

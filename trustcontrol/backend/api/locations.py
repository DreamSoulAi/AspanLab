...
# Стартовый глоссарий по типу бизнеса — подгружается при создании точки
# если владелец не задал свои слова. Слова из базового STT-промпта не дублируем
# (Каспи/QR/терминал/наличные/чек/сдача/теңге/сәлем/рахмет уже в stt_prompt.py).
_SEED_PHRASES: dict[str, list[str]] = {
    "coffee":   ["капучино", "американо", "латте", "флэт уайт", "эспрессо", "раф",
                 "S", "M", "L", "сироп", "добавка", "оат", "кокосовое", "большой", "маленький"],
    "cafe":     ["лагман", "манты", "самса", "плов", "шурпа", "шашлык", "борщ",
                 "порция", "блюдо", "гарнир", "чай", "компот"],
    "fastfood": ["бургер", "донер", "шаурма", "картошка", "соус", "стрип", "комбо",
                 "большой", "маленький", "острый", "без лука"],
    "gas":      ["бензин", "дизель", "92", "95", "98", "литров", "бак", "полный",
                 "ГСМ", "масло", "антифриз"],
    "shop":     ["упаковка", "штука", "кило", "пачка", "акция", "скидка",
                 "пакет", "сумка", "чек"],
    "beauty":   ["стрижка", "маникюр", "педикюр", "окраска", "укладка", "мастер",
                 "запись", "минут", "тонирование"],
    "fitness":  ["абонемент", "заморозка", "тренер", "гостевой", "посещение",
                 "месяц", "разовый", "заморозить"],
    "hotel":    ["номер", "заезд", "выезд", "завтрак", "бронь",
                 "двухместный", "одноместный", "сутки"],
    "pharmacy": ["таблетки", "мазь", "сироп", "капли", "рецепт",
                 "дозировка", "упаковка", "аналог"],
    "clinic":   ["талон", "приём", "врач", "анализ", "запись",
                 "кабинет", "результат", "направление"],
    "auto":     ["замена масла", "шиномонтаж", "диагностика", "запчасти",
                 "мойка", "балансировка", "фильтр"],
    "service":  ["мастер", "срок", "замена", "ремонт", "стоимость",
                 "детали", "гарантия"],
}
...
async def create_location(
    ...
    loc = Location(
        ...
        custom_phrases=_SEED_PHRASES.get(data.business_type, []),
    )
    db.add(loc)
    await db.commit()

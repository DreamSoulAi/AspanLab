...
def has_transfer_intent(text: str) -> bool:
    """True если в тексте звучит просьба ПЕРЕВЕСТИ деньги на номер (не обычная QR-оплата)."""
    tl = (text or "").lower()
    return any(phrase in tl for phrase in _TRANSFER_PHRASES)

...
2. Реплика есть в одном и нет в другом → ВКЛЮЧИ её (если правдоподобна). Итог
   должен содержать ВСЁ сказанное из ОБОИХ вариантов — это наложение двух
   распознаваний ОДНОЙ записи, а не выбор одного варианта вместо другого.
3. 🔴 СУММЫ И ОПЛАТА — НИКОГДА НЕ ТЕРЯЙ. Если в ЛЮБОМ из вариантов есть сумма
   (цифры рядом с тенге/теңге/мың/тысяч/₸/тг) или способ оплаты (Каспи, QR,
   перевод, аудар, наличные, карта, картамен, сдача, терминал) — ОБЯЗАТЕЛЬНО
   перенеси в итог дословно, даже если этой строки нет в «предпочитаемом» по
   языку варианте. От сумм и оплаты зависит выявление мошенничества — их потеря
   недопустима.
4. Исправляй фонетические ошибки по контексту (рожок, стакан, QR, Каспи, аудар).
5. Если вариант явно галлюцинирует (несвязный/выдуманный) — используй другой.
6. НЕ придумывай слов которых не было ни в одном варианте.
7. Сохраняй язык каждого слова.
...
async def _merge_transcripts(
    ...
    # Страховка от потери транзакции: если сумма/оплата БЫЛИ в исходнике, но
    # пропали из склейки — merge проглотил критичные для фрода данные (так было на
    # смешанной каз/рус записи: gpt-4o дал «Сізден 1900 тенге…», а склейка взяла
    # казахскую версию ISSAI и сумму выкинула). Молча не доверяем: помечаем
    # needs_review и роняем уверенность, чтобы в отчёте было видно — склейке тут
    # верить нельзя. Текст не правим (вставка вслепую рискованнее, чем флаг).
    src_had_money = (_looks_like_real_transaction(issai_clean)
                     or _looks_like_real_transaction(openai_clean))
    if src_had_money and not _looks_like_real_transaction(result["text"]):
        log.warning(
            "Merge потерял сумму/оплату из исходника — needs_review | "
            f"issai={issai_clean[:60]!r} openai={openai_clean[:60]!r} "
            f"→ merged={result['text'][:60]!r}"
        )
...
async def analyze_audio_with_fallback(
    ...
    # → таймауты → потеря записей. Теперь:
    #   • OpenAI и ISSAI запускаются вместе;
    #   • OpenAI (надёжный, ~10-30с) ждём всегда — это основа;
    #   • ISSAI добираем в пределах окна ISSAI_GRACE_SECONDS. Уложился (свободен)
    #     → сливаем с OpenAI ради лучшего казахского. Не уложился (занят/очередь)
    #     → отменяем и работаем на OpenAI. Латентность ограничена окном, а не
    #     глубиной очереди ISSAI. Триаж-гейты экономии убраны: на пике важнее
    #     надёжность, personal/noise всё равно отсекает text-GPT ниже.
    if issai_stt.is_enabled():
        import time as _time
        issai_diag: dict = {}
        _t0 = _time.monotonic()
        issai_task = asyncio.create_task(issai_stt.transcribe(wav_bytes, diag=issai_diag))
        openai_task = asyncio.create_task(_transcribe_audio(
            wav_bytes, model=_PRIMARY_STT_MODEL, location_glossary=location_glossary
        ))

        # OpenAI — ждём всегда (надёжная основа)
        primary_raw = _strip_repeat_loops(await openai_task or "")
        log.info(f"Параллель | OpenAI STT: {len(primary_raw)} симв ({primary_raw[:60]!r})")

        # ISSAI — best-effort в пределах общего окна от старта обоих задач
        issai_raw = ""
        _remaining = settings.ISSAI_GRACE_SECONDS - (_time.monotonic() - _t0)
        if _remaining > 0:
            try:
                issai_raw = _strip_repeat_loops(
                    await asyncio.wait_for(issai_task, timeout=_remaining) or ""
                )
                log.info(f"Параллель | ISSAI: {len(issai_raw.split())} сл ({issai_raw[:60]!r})")
            except asyncio.TimeoutError:
                issai_task.cancel()
                log.info(f"ISSAI не успел за {settings.ISSAI_GRACE_SECONDS:.0f}с (занят/очередь) — работаем на OpenAI")
                issai_diag.update({"engine": "issai", "stage": "grace_timeout"})
            except Exception as e:
                log.warning(f"ISSAI ошибка в параллельном пути: {e}")
                issai_diag.update({"engine": "issai", "stage": "error", "error": str(e)[:160]})
        else:
            issai_task.cancel()
            log.info("ISSAI пропущен — окна не осталось (OpenAI ответил позже грейса)")
            issai_diag.update({"engine": "issai", "stage": "grace_skip"})

        # ── Объединение и финальный анализ ───────────────────────────────────
        if issai_raw or primary_raw:
            ...
            if merged and len(merged.split()) >= 2:
                stt_diag = {
                    "engine":      "parallel_hybrid" if (issai_raw and primary_raw) else _PRIMARY_STT_MODEL,
                    "issai":       issai_raw[:120],
                ...
                log.info(f"Параллель merged | {merged[:80]!r}")

...
_MERGE_PROMPT = """Перед тобой ДВЕ транскрипции ОДНОЙ И ТОЙ ЖЕ аудиозаписи с кассы в Казахстане.
...
async def _merge_transcripts(
    ...
    try:
        ...
        if merged and len(merged.split()) >= 2:
            # Страховка от склейки встык: ISSAI и OpenAI — это ОДНО аудио.
            # Если GPT всё-таки дописал один за другим, merged ≈ сумме длин обоих
            # → разговор задвоен. Не доверяем, берём один (более длинный) транскрипт.
            longest = max(len(issai_clean), len(openai_clean))
            if len(merged) > longest * 1.4:
                log.warning(
                    f"Merge выглядит как склейка встык (merged={len(merged)}ч > "
                    f"1.4×{longest}ч) → беру одиночный транскрипт без задвоения"
                )
                return _strip_repeat_loops(
                    issai_clean if len(issai_clean) >= len(openai_clean) else openai_clean
                )
            log.info(
